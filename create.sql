create table tb_cliente (id bigint generated by default as identity, cpf varchar(255) not null, data_nascimento DATE, email varchar(255), nome_completo varchar(255), rg varchar(255), telefone varchar(255), conta_id bigint, endereco_id bigint, primary key (id));
create table tb_conta (id bigint generated by default as identity, banco varchar(255), numero_agencia varchar(255), numero_conta varchar(255), saldo double, primary key (id));
create table tb_conta_extratos (conta_id bigint not null, extratos_id bigint not null);
create table tb_endereco (id bigint generated by default as identity, cep varchar(255), cidade varchar(255), complemento varchar(255), estado varchar(255), numero integer, rua varchar(255), primary key (id));
create table tb_extrato (id bigint generated by default as identity, momento timestamp, nome varchar(255), operacao varchar(255), valor double, primary key (id));
alter table tb_cliente add constraint UK_jgra977gi05fur83l225x4qkr unique (cpf);
alter table tb_conta_extratos add constraint UK_1loljoemew82bbd6bdy8s63w9 unique (extratos_id);
alter table tb_cliente add constraint FKswcqckm9tp290kdbawo52d809 foreign key (conta_id) references tb_conta;
alter table tb_cliente add constraint FK69e5s70uca4u6fhiatenwcyk1 foreign key (endereco_id) references tb_endereco;
alter table tb_conta_extratos add constraint FK9xxac4988uel1j0m389gkvbu5 foreign key (extratos_id) references tb_extrato;
alter table tb_conta_extratos add constraint FKdm07y4fby6kpbo106poba3srw foreign key (conta_id) references tb_conta;
